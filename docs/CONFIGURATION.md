...
Hausverbrauch und Hausverbrauch ohne Wallbox werden zentral aus dem Datenstand des Regelzyklus geschrieben. Die beiden früher vom Modul angelegten Berechnungsereignisse werden beim Anwenden deaktiviert, nicht gelöscht. Ergebnisvariablen und IDs bleiben erhalten. Ihre Aktualisierung folgt dem Regelintervall; bei ausgeschaltetem Modul oder deaktivierter Abfrage ohne Fahrzeug erfolgt keine unabhängige Ereignisaktualisierung.

Bei konfigurierter Hausverbrauchsquelle überwacht das Modul deren VariableUpdated und die Wallboxleistung. Eine Änderung der Wallboxleistung um mehr als 150 W gegenüber der Referenz oder ein unplausibler Hauswert (mehr als 100 W unter Wallboxleistung, aus der Zukunft oder zu alt) verwirft die zeitliche Plausibilität. Zur erneuten Freigabe sind zwei unterschiedliche spätere Hausverbrauchsaktualisierungen bei ausreichend stabiler Wallboxleistung erforderlich. Das maximal akzeptierte Messwertalter und die maximale Wartezeit betragen jeweils max(120 Sekunden, 3 × RefreshInterval). Dies sind feste Beta-Regeln, keine zusätzlichen Einstellungen.

Währenddessen bleiben die letzten plausiblen Haus-Anzeigewerte erhalten. PV, PV-Anteil und Hybrid speisen den verzögerten Wert nicht in Filter/Glättung ein, starten nicht neu und fordern keine überschussbedingten Stromerhöhungen oder Phasenwechsel an. Laufende Ladung bleibt höchstens auf dem aktuellen Strom; Netzbegrenzung und Stopbedingungen behalten Vorrang. Bereits angeforderte Phasenwechsel dürfen im Stillstand bestätigt werden, ohne erneute Freigabe. Nach Ablauf der Wartezeit wird Stop angefordert. Manuell bleibt unabhängig von dieser PV-Wartebedingung; seine Netz-/SoC-Prüfungen gelten unverändert.

VariableUpdated bestätigt den Eingang in Symcon, nicht den tatsächlichen Messzeitpunkt eines vorgeschalteten Zählers. Zwei Aktualisierungen begrenzen typische Verzögerungen, garantieren aber keine physikalische Gleichzeitigkeit. Eine Quelle ohne Wallboxanteil oder mit längerem internem Berechnungsverzug muss weiterhin korrigiert werden.
...
Die Fahrzeugphasen werden automatisch aus drei zeitlich getrennten Strommessungen während aktiver Ladung im 3P-Wallboxmodus erkannt. Bis zur Bestätigung wird konservativ mit drei Phasen gerechnet. Eine geänderte Anzahl, Ladepause, Abstecken, Phasenwechsel, Kommunikationsfehler oder längere Messlücke verwirft die Erkennung. Im 1P-Modus wird immer mit einer Phase gerechnet. Die frühere Einstellung CarMaxPhases ist ausgeblendet und wird nicht mehr ausgewertet. Die Erkennung gilt für alle Leistungsberechnungen; die Wallbox schaltet weiterhin ausschließlich 1P/3P. Eine spontane Änderung der tatsächlich genutzten Phasen kann erst beim nächsten Statusabruf berücksichtigt werden.
